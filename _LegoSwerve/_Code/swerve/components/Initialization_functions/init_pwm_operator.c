...
void init_pwm_operator(motor_data_t *motor_data, const Motor_Config_t *motor_config) {
    ...
    ESP_ERROR_CHECK(mcpwm_new_comparator(oper, &comparator_config, &motor_data->pwm_comparator));
    ...
    ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(motor_data->pwm_comparator, map_speed_to_pulsewidth(0)));
    ...
    ESP_ERROR_CHECK(mcpwm_generator_set_action_on_compare_event(generator,
                                                                MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, motor_data->pwm_comparator, MCPWM_GEN_ACTION_LOW)));
